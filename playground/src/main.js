...
function render() {
  ...
  document.querySelector('#app').innerHTML = `<main class="page">
    ...
    <section class="grid"><div class="card editor"><h2>${t.input}</h2><table><thead><tr>${recipe.headers.map((header) => `<th>${header}</th>`).join('')}</tr></thead><tbody>${values.map((row, rowIndex) => `<tr>${row.map((value, colIndex) => `<td>${typeof value === 'number' ? `<input class="cell-input" data-row="${rowIndex}" data-col="${colIndex}" type="number" value="${value}" aria-label="${recipe.headers[colIndex]} ${rowIndex + 1}" />` : `<input class="cell-input" data-row="${rowIndex}" data-col="${colIndex}" type="text" value="${value}" aria-label="${recipe.headers[colIndex]} ${rowIndex + 1}" />`}</td>`).join('')}</tr>`).join('')}</tbody></table><div class="actions"><button id="calculate">${t.calculate}</button><button class="secondary" id="reset">${t.reset}</button></div></div>
