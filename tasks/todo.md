...
現在のテスト総数: **461 件**（lib）+ `tests/cli_json.rs` 14 件 + `tests/cli_check.rs` 15 件 + `tests/blackbox.rs` 1 件（内部で `tests/fixtures/blackbox/*.toml` 12 件を走査）+ `tests/cli_snapshot.rs` 7 件 + `tests/cli_test_workbook.rs` 7 件
