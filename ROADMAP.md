...
3. **readerのOSシグナル連携と実測校正が未完了** — 総work budget、chunk単位のdeadline/cancellation、
   CLIのcancel-fileは実装済みだが、OSシグナル連携と実測校正は残っている。
...
- [x] ZIP entry数、entryごとの展開後サイズ、総展開サイズ、圧縮率を上限付きで検査する。
- [x] XMLの要素数、属性数、属性値/テキスト長、共有文字列数、シート/行/列/セル数を制限する。
- [x] XML 1.0で許可されない制御文字を通常reader・stream reader共通で拒否する。
- [x] XML制御文字の拒否条件を`docs/limits.md`と`docs/xlsx-security-model.md`へ反映する。
- 外部エンティティ、外部DTD、外部参照、想定外のencoding、異常なrelationship targetを拒否する。
- [x] XMLの処理命令を`?>`で終端される場合だけ受け入れ、未知の`<!...>`宣言や未終了構文を無視せず拒否する。
- [x] パスを正規化し、出力先・relationship・ZIP entryから作業ディレクトリ外へ出られないようにする。
...
- [x] Rust/Pythonのreader APIに`max_work_units`、`timeout_ms`、協調キャンセルtokenを提供し、CLI snapshotからbudget/deadline/cancel-fileを指定できるようにする（OSシグナル連携は未完了）。
