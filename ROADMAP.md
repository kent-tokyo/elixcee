...
- [x] G2d Drawing text regression: 最小Drawing-backed XLSXの保存後ZIP再読込で、選択anchorの先頭および0-based指定text runのXML escape付き更新を確認した。先行実装時の低ディスクRust workspace全targetは1,680 library testsと全integration／benchmark targetが成功し、follow-upのindexed-run追加は対象単体・保存往復・clippyで検証した。[G2d測定](docs/measurements/g2d-object-editing-2026-09-09.md)
