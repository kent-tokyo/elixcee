...
- [x] G2d Drawing shape-name BUILD: 既存two-cell anchor内の`<xdr:cNvPr name>`だけを`Vm.set_drawing_shape_name`（Python binding／型stub含む）からXML escape付きで更新できるようにした。geometry・shape content・relationshipは保持し、Chart/Drawing作成、shape全般、Excel再openは未完。
