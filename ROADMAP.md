...
- [x] G4 spill再計算API: `recalculate_all_with_spills()`でactive sheetのformula再計算、配列anchorの事前衝突検査／一括spill適用、依存先の再計算を接続した。既存`recalculate_all()`は互換性のため変更せず、別sheet・複数行shape・Excel oracle校正は未完。
