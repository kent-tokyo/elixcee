...
- [x] G2d Chart axis-title BUILD: 既存cat／val／date／ser軸のdocument-order指定について、`Vm.set_chart_axis_title`（Python binding／型stub含む）から軸内の最初のtitle text runだけを更新できるようにした。軸タイトル欠損、Chart作成、Excel再openは未完。
