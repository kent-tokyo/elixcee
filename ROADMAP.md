...
- [x] G4 独立formula oracle拡張: LibreOfficeと同一生成fixtureで丸め（FLOOR／CEILING／MROUND）・日付境界（YEAR／MONTH／DAY／EDATE／EOMONTH）を追加校正し、47/47 comparableでelixceeと一致した。DAYS／IFNA／XMATCH／TEXTJOINは当該LibreOffice buildの未評価として4件を明示skip。これはExcel oracle、全型変換、1904 serial補正、配列境界の完了を意味しない。[測定記録](docs/measurements/formula-independent-oracle-2026-09-10.md)
