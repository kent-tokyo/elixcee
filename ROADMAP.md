...
- [ ] Workbook_Open／Worksheet_Change等のイベントは、EnableEvents・再入抑止・決定的dispatch順・budget付きで設計。明示dispatch、Target binding、自動発火、boundedイベント連鎖、重複handler拒否はBUILD済みで、worksheet単位の複数handler意味論とExcel oracleが残る。
