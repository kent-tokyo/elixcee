...
- [x] G2d Chart legend BUILD: 既存Chart XMLの最初の`<c:legendPos@val>`を`Vm.set_chart_legend_position`（Python binding／型stub含む）から`b`／`tr`／`r`／`l`／`t`の範囲で限定更新できるようにした。legend以外のChart XML・Drawing／relationship chainは保持し、凡例作成・Chart作成・Excel再openは未完。
