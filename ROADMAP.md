...
- [x] G2d Chart reopen probe: データ行のみを参照するbar chart（`A2:A6`／`B2:B6`）もmacOS Excelで修復警告となることを確認した。ヘッダー混在が原因ではないため、生成ChartのExcel再openは未検証のまま維持する。[測定記録](docs/measurements/g2d-object-editing-2026-09-09.md)
...
- [x] G2d Chart data-label category BUILD: 既存Chartの最初の`c:dLbls@showCatName`を`Vm.set_chart_data_labels_show_category`（Python binding／型stub含む）からboolean OOXML値で追加／更新できるようにした。showValとの併用を保持し、data-label要素の新規作成、他のラベル属性、Chart作成、Excel再openは未完。
- [x] G2d Chart data-label series-name BUILD: 既存Chartの最初の`c:dLbls@showSerName`を`Vm.set_chart_data_labels_show_series_name`（Python binding／型stub含む）からboolean OOXML値で追加／更新できるようにした。showVal／showCatNameとの併用と既存子要素を保持し、data-label要素の新規作成、Chart作成、Excel再openは未完。
...
- [x] 部分 G4 Excel oracle: Microsoft Excel for Mac 16.108で14 probe（MAXIFS／MINIFS／DAYS／IFNA／XMATCH／TEXTJOIN、丸め・1900日付境界・Empty/Error・配列shape）を自動計算し、現行source wheelの同一式と14/14一致を確認した。これは14 probe・1 hostの校正であり、全関数・全型変換・他OSのExcel互換性を意味しない。[測定記録](docs/measurements/formula-excel-oracle-2026-09-11.md)
- [x] G4 compatibility fix: Excel oracleで`AVERAGE("1",2)`が1.5になることを確認し、elixceeの直接引数coercionを修正した。参照範囲内の文字列・論理値を無視する既存の区別は維持し、修正後source wheelで15 probe全件を再計算した。[測定記録](docs/measurements/formula-excel-oracle-2026-09-11.md)
...
- [x] 部分 G2d Chart cache BUILD: 新規Chartの単純なworksheet範囲からcategory/value cache pointを生成し、数値cacheへ文字列を混入させない回帰を追加した。Excel再openの修復警告はdata-row probeでも残るため、Chart/Drawing互換性の完了扱いにはしない。[測定記録](docs/measurements/g2d-object-editing-2026-09-09.md)
...
- [x] 現行ソース再確認: CPython 3.13 arm64 wheelでappend Writerを100,000／250,000行×3列、各2回、さらに1,000,000行を1回測定した。RSSはそれぞれ19.88／19.88／19.93 MiBで、全件ZIP・worksheet・semantic digestに成功した。これはmacOS append経路の再現確認であり、通常VM、Linux／Windows、Excel oracle、G5全体の完了を意味しない。[記録](docs/measurements/writer-streaming-2026-09-11.md)
- [x] G5 append repeatability follow-up: 同一macOS arm64 wheelを100,000／250,000行×3列、各3回再測定し、RSS p50/p95 19.86/20.03 MiB、19.90/20.00 MiB、全6出力のZIP／worksheet／最終行／semantic digest検証成功を確認した。Linux／Windows、通常VM、1M p95、G5全体の完了を意味しない。[記録](docs/measurements/writer-streaming-2026-09-11.md)
- [x] G5 normal-fresh repeatability follow-up: 同一条件でnormal-freshを100,000／250,000行×3列、各3回再測定し、RSS p50/p95 117.11/117.14 MiB、204.30/204.30 MiB、wall p50/p95 213/231 ms、508/540 ms、全6出力のsemantic検証成功を確認した。通常VMのRSSは行数依存であり、Linux／Windows、1M p95、G5全体の完了を意味しない。[記録](docs/measurements/writer-streaming-2026-09-11.md)
...
- [x] 1.05x large-workbook follow-up: allocation-free A1 parsing, formula-free
  load fast path, optional XML rescan skips, and event-only worksheet codeName
  loading reached 1.060x/1.074x on strict 100k/400k pairs. The 1M supporting
