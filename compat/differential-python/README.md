...
- `sheet_ops_check.py`: sheet management (P1 core 3 + remainder + P2 hidden
  row/col + copy_sheet) — `rename_sheet`/`move_sheet`/`copy_sheet`/
  `merged_cells`/`merge_cells`/`unmerge_cells`/`hidden_rows`/
  `hidden_columns`/`set_row_hidden`/`set_column_hidden`, plus PyO3-layer
