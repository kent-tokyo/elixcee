...
### Root crate: standalone `<autoFilter>` filtering — 0.16.0-B1

First of two AutoFilter sub-slices (0.16.0-B2, table-embedded filter criteria, is a small
fast-follow reusing this one's engine). Real filter creation/evaluation, distinct from PR
#29's earlier byte-preservation fix for an already-present `<autoFilter>` — this ships
actually setting criteria and computing the resulting row-hide state.

- `vm.add_autofilter(ref, sheet=None)` / `vm.remove_autofilter(sheet=None)` /
  `vm.set_equality_filter(col_offset, values, sheet=None)` /
  `vm.set_custom_filter(col_offset, operator, value, and_=True, operator2=None,
  value2=None, sheet=None)` / `vm.set_blank_filter(col_offset, sheet=None)` /
  `vm.set_top10_filter(col_offset, val, top=True, percent=False, sheet=None)` /
  `vm.set_date_group_filter(col_offset, year=None, month=None, day=None, hour=None,
  minute=None, second=None, grouping="day", sheet=None)` /
  `vm.clear_filter_column(col_offset, sheet=None)` / `vm.autofilter(sheet=None)`
  (read-only). `col_offset` is 0-based relative to the autofilter's own `ref` left edge —
  not the same convention as VBA `RangeAutoFilter`'s 1-based `Field`.
- All five real `<filterColumn>` shapes (equality/multi-value list, one-or-two-condition
  custom numeric/text comparison, blanks, top/bottom N-or-percent, date-group buckets)
  verified against real `openpyxl`-generated bytes for every variant before writing any
  parser/writer code — `ST_FilterOperator` has no `between`/`notBetween` (that's a
  data-validation-only operator set, a real prior assumption corrected by checking rather
  than reusing `DataValidationRule`'s own operator list unverified); a numeric/date range
  is two `<customFilter>` conditions joined by `and="1"` instead.
- **Real row-hide evaluation, not persist-only** (unlike 0.16.0-C's data validation,
  which never evaluates): every `set_*_filter`/`clear_filter_column` call immediately
  re-evaluates ALL currently-active columns together against real cell data and rewrites
  the full hidden-row set for the autofilter's data rows in one pass, via the existing
  `set_row_hidden_on_sheet` API — never incremental, since multiple `<filterColumn>`s
  combine via AND (clearing one column's filter while another is still active could
  otherwise leave hidden a row that only ever failed the cleared criterion). One-shot,
  matching `Stmt::RangeAutoFilter`'s own real VBA behavior — not live/continuous
  re-evaluation as cell values later change. `Top10` needs the full column's values to
  compute a threshold before any row can be judged, so it's precomputed in its own pass
  first; every other type is evaluable per-row independently. Date-group matching reuses
  `elixcee_types::serial_to_ymd` (the same decomposition already used for VBA's
  `Now()`/`DateSerial`) for the calendar part, with the sub-day fraction decomposed into
  hour/minute/second directly.
- Write path follows Tables' own A2-Addendum lesson (surgical patch of the ORIGINAL raw
  bytes via `with_attr`/`with_ordered_child`, not reserialize-from-struct) rather than
  0.16.0-C's regenerate-container pattern — a real `<filterColumn>` may carry an `extLst`
  this project's model doesn't capture. An untouched `<autoFilter>` on an unrelated save
  stays exactly what PR #29 already guaranteed, byte-identical.
- `Vm.autofilters`/`autofilters_touched` (per-sheet, at most one autofilter per sheet per
  `CT_Worksheet`'s own `maxOccurs="1"`, unlike `tables`) added to `rename_sheet`
  (now 23 re-keyed maps)/`remove_sheet`/`copy_sheet`'s existing per-sheet-map handling.
  `ref` shifts via the same `formula::rewrite` primitives as every other range-shaped
  field; a column-axis edit also renumbers each `FilterColumn::col_offset` (converted to
  an absolute column, shifted via `shift_cell_coord`, dropped if deleted, reconverted
  against the new `ref`'s left edge) — marked dirty only when the offset actually changes
  numerically, so an edit outside `ref` (which shifts every column's absolute position by
  the same delta) leaves untouched columns' raw bytes byte-identical.
- Verification exception granted for all five filter-criteria shapes (zero real-Excel
  fixture grounding for any active filter anywhere in this project) — same basis as every
  prior grant this session, verified here against real `openpyxl`-generated bytes for
  every type, not spec text alone.
- `remove_autofilter` reveals every row in the (former) filtered range currently hidden,
  matching real Excel's "Clear Filter" behavior — disclosed, not fixed: this project's
  hidden-row state carries no "hidden by filter" vs. "hidden by an explicit hide call"
  provenance, so this could theoretically also reveal an unrelated hide if it happens to
