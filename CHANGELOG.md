...
- G2dの限定Chart編集として、既存軸のタイトルを`Vm.set_chart_axis_title`（Python binding／型stub含む）からdocument orderの0-based indexで更新できるようにしました。軸内の最初のtitle text runだけを変更し、系列・Chart title・relationshipは保持します。軸タイトルの新規作成、Chart作成、Excel再openは未完です。
