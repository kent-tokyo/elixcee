...
- **`@elixcee/xlsx` Phase 1A-1B-2B**: `packages/xlsx` grew from an empty skeleton into a working `xlsx@0.18.5`-compatible utility surface, differential-tested against the real oracle throughout (400+ permanent fixtures across `compat/differential/`) —
  - Address/workbook utilities (`encode_*`/`decode_*`, `book_new`, `book_append_sheet`, `book_set_sheet_visibility`), worksheet mutation (`aoa_to_sheet`/`sheet_add_aoa`, `json_to_sheet`/`sheet_add_json`), formula extraction (`sheet_to_formulae`), cell metadata (`cell_set_hyperlink`, `cell_set_internal_link`, `cell_add_comment`, `sheet_set_array_formula`), and text export (`sheet_to_csv`, `sheet_to_txt`)
  - `format_cell`/`cell_set_number_format` are backed by the real `ssf@0.11.2` engine (Phase 1B-2B) — `packages/xlsx`'s first runtime dependency, isolated behind a single adapter file (see `docs/xlsx-architecture.md`'s "SSF backend" decision and `THIRD_PARTY_NOTICES.md`); one genuine upstream indirection-table bug (numFmtIds 67-71) was found and corrected in that adapter
  - Two DoS-shaped divergences from the real oracle, both empirically confirmed (not assumed) and registered as intentional safety divergences: `encode_col(Infinity)` (the oracle hangs; elixcee rejects non-finite indices) and a crafted full-grid `!ref` fed to `sheet_to_formulae`/`sheet_to_csv`/`sheet_to_txt` (the oracle takes 12s+ / doesn't return within 25s; elixcee caps iteration at 5,000,000 cells)
  - One security fix: `book_append_sheet` no longer lets a sheet literally named `"__proto__"` reassign `wb.Sheets`'s own prototype (the real oracle's plain bracket-assignment does)
  - Still explicitly out of scope: `sheet_to_json`/`table_to_sheet`/`table_to_book`/`sheet_to_html`, XLSX/ODS file reading or writing (`read`/`readFile`/`write*`), any Rust↔JS bridge, and npm publish
