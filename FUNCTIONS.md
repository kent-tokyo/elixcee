...
Coverage reference for **elixcee 1.0.10**. See [CHANGELOG](CHANGELOG.md) for versioned changes.
...
`Scripting.Dictionary` is active in the **1.0.10 VM-local adapter** (introduced in
1.0.9), not a COM object.
...
| `TTEST` / `T.TEST` | Student's t-test probability for paired or two-sample data | Classic / 2010 |
...
| `ISOMITTED` | Return `TRUE` for an omitted AST argument or missing higher-order LAMBDA binding; explicit `Empty` remains `FALSE` | 2024/365 |
