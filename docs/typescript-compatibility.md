...
Confirmed by diffing the real runtime `Object.keys(XLSX.utils)` (33 keys) against both the
types file's `XLSX$Utils` interface and elixcee's own current exports — a mechanical
comparison, not a guess:

- `table_to_sheet`, `table_to_book`, `sheet_add_dom`, `sheet_to_html` — real runtime
  exports, declared in the oracle's types (`Table2SheetOpts`, `Sheet2HTMLOpts`). Phase 1C
  target.
- `consts` — real runtime export (`{SHEET_VISIBLE:0, SHEET_HIDDEN:1,
  SHEET_VERY_HIDDEN:2}`, confirmed live), declared in the oracle's types as
  `consts: XLSX$Consts`. **Note the runtime/types name mismatch already present in the
  oracle itself**: the types file declares the third key as `SHEET_VERYHIDDEN` (no
  underscore before "HIDDEN"), but the real runtime object's own key is
  `SHEET_VERY_HIDDEN` (underscore) — confirmed via `Object.getOwnPropertyDescriptor`,
  not assumed from the `.d.ts`. Whichever phase implements this must reproduce the
  RUNTIME key name, and record the oracle's own types-vs-runtime inconsistency here
  rather than "fixing" it. Phase 1C target.
- `sheet_to_row_object_array` — real runtime export, confirmed to be a literal alias for
  `sheet_to_json` (`U.sheet_to_row_object_array === U.sheet_to_json` is `true`), but not
  declared anywhere in the oracle's own `types/index.d.ts`. Phase 1C target (the
  "remaining utility aliases" commit).
- `sheet_to_dif`, `sheet_to_slk`, `sheet_to_eth` — declared in the oracle's types
  ...
  mirror image of the `SHEET_VERY_HIDDEN` case above). Not a Phase 1C target: there is no
  real runtime function to differential-test against, so implementing these would mean
  inventing behavior with no oracle to verify against — out of scope until/unless a future
  oracle version actually ships them.
