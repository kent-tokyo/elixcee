...
| `sheet_to_html` | EXTENDED | `Sheet2HTMLOpts` mirrors the oracle's `id`/`editable`/`header`/`footer` fields and adds `rawHtml?: boolean` as an explicit security opt-in for trusted `cell.h` markup. |
