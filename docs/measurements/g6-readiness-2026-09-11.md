...
| G3 workbook formulas | Rust regression suite plus workbook-level dirty-closure, cycle, rename, and event tests; six real Excel formula probes now match. | Partial: implementation evidence exists, but complete Excel semantic oracle coverage is not established. |
...
| G5 memory | macOS arm64 append and normal-fresh measurements cover 100k/250k/1M rows and semantic output equality; the current 1.0.10 wheel also passed the 2026-09-11 append rerun at 100k/250k rows. | Partial: Linux/Windows matrix and full passthrough-laziness gate are not run. |
